...
filtered_cities["latitude"] = pd.to_numeric(
    filtered_cities["latitude"], errors="coerce"
)
filtered_cities["longitude"] = pd.to_numeric(
    filtered_cities["longitude"], errors="coerce"
)

viet_coord_data["latitude"] = pd.to_numeric(
    viet_coord_data["latitude"], errors="coerce"
)
viet_coord_data["longitude"] = pd.to_numeric(
    viet_coord_data["longitude"], errors="coerce"
)
...
def search_city(
    ...
    for _, row in filtered_cities.iterrows():
        ...
        if (
            name == ascii_name
            or name == std_name
            or name in [alt.strip() for alt in alt_names]
        ):
            return (
                row["name"],
                f"The city '{row['asciiname']}' is found in the dataset, we use this as current city.",
            )
    ...
    if nearby.empty:
        return (
            None,
            f"No nearby cities within {coord_threshold} degrees found near '{name}'.",
        )

    # Step 4: Compute Euclidean distance
    nearby = nearby.copy()
    nearby["euclid_dist_deg"] = np.sqrt(
        (nearby["latitude"] - city_lat) ** 2 + (nearby["longitude"] - city_lon) ** 2
    )
    ...
    return (
        closest["name"],
        f"The closest city with data is '{closest['asciiname']}', which is approximately {dist_km:.2f} km from '{name}'.",
    )
...
def toggle_notification(key):
    st.session_state.notificationsEnabled[key] = (
        not st.session_state.notificationsEnabled[key]
    )
...
def render_welcome(_=None):
    ...
    if version == "data_saving":
        st.info(
            'The current versions is the data saving version, if you want acces to pictures, please change the setting with the button "Version and Locations" '
        )
...
def version_1(_=None):
    st.header(
        f"Select the version or location you want(current version: {st.session_state.version_show} and current location: {st.session_state.loc})"
    )
    ...
    st.button(
        "Data Saving Version",
        on_click=partial(set_version, "data_saving", "Data Saving Version"),
    )
    st.button(
        "Performance Optimised Version",
        on_click=partial(set_version, "performance", "Performance Optimised Version"),
    )
    st.button(
        "Extension Officers Version",
        on_click=partial(set_version, "extension", "Extension Officers Version"),
    )
...
def set_location(_=None):
    ...
    st.header(
        f"Select your location (your current location is: {st.session_state.loc}). "
    )

    with st.form(
        key="location_form"
    ):  # I do not prefer the form format, but for now, it is neat.
...
def weather_forecast_period(_=None):
    ...
    def select_period(months):
        ...
        filename = (
            f"forecast_graph_{city.replace(' ', '_').lower()}_{months}_months.png"
        )
    ...
    for p in periods:
        st.button(
            f"{p} week{'s' if p > 1 else ''}", on_click=partial(select_period, p)
        )  # now weeks
    back_button()
...
def weather_forecast_graph(_=None):
    ...
    st.header(
        f"Weather in {city} - Last {months} week{'s' if months > 1 else ''}"
    )  # now weeks
...
def weather_info(_=None):
    ...
    if loc in todays_climate_data:
        ...
        def load_advice(category, classification):
            mapping = {"below average": "0", "about average": "1", "above average": "2"}
    ...
    if version == "performance":
        ...
        st.button(
            "Go to forecasts", on_click=partial(navigate, "weather_forecast_period")
        )
        ...
        st.info(
            "Weather graphics are not available in this version to save data. Change version for the forecasts"
        )
...
def crop_advice_1(_=None):
    ...
    st.button(
        "Pest and diseases",
        on_click=partial(navigate, "crop_advice_2", "pest_and_diseases"),
    )
    st.button("Back to the main menu", on_click=partial(navigate, "welcome"))
...
def crop_advice_2(type_):
    if type_ == "pest_and_diseases":
        ...
        for crop in crops:
            crop_key = crop.lower().replace(" ", "_")
            st.button(
                crop, on_click=partial(navigate, "crop_cultivation_adv", crop_key)
            )
    back_button()
...
def price_info_1(_=None):
    st.header(
        "Welcome to the price menu, these are the most recent prices for the crops. "
    )

    for crop in crops:
        price = cropPrices.get(crop)
        label = (
            f"{crop} - {price:.2f} usd per kg"
            if price is not None
            else f"{crop} - (no price available)"
        )  # this can be converted to vnd
        st.write(label)
    st.button("Back to the main menu", on_click=partial(navigate, "welcome"))
...
def GAP_1(_=None):
    ...
    st.button(
        ...
        on_click=partial(navigate, "GAP_2", "Conservation_agriculture"),
    )
...
def GAP_2(type_):
    if type_ == "Conservation_agriculture":
        ...
        st.button(
            ...
            on_click=partial(navigate, "GAP_2", "three_principles"),
        )
        st.button(
            "Step by Step guide", on_click=partial(navigate, "GAP_2", "SBS_guide")
        )
        back_button()
        ...
        st.button(
            "Back to the alternative farming methods",
            on_click=partial(navigate, "GAP_1"),
        )
...
def notifications_1(_=None):
    st.header(
        "What type of notifications would you like to receive? Click to switch them on or off."
    )
    ...
    st.button(
        weather_label,
        key="notif_weather",
        on_click=partial(toggle_notification, "weather"),
    )

    st.button(
        "Crop Cultivation",
        on_click=partial(navigate, "notifications_2", "crop_cultivation"),
    )  # time dependent push notifications
    st.button(
        "Price Updates", on_click=partial(navigate, "notifications_2", "price_updates")
...
def toggle_weather_alerts():
    st.session_state.notificationsEnabled["weather"] = (
        not st.session_state.notificationsEnabled["weather"]
    )
    ...
    if type_ == "crop_cultivation":
        ...
        for crop in crops:
            ...
            st.button(
                label,
                key=f"notif_{crop}",
                on_click=partial(toggle_notification, crop_key),
            )
        ...
        for crop in crops:
            ...
            st.button(
                label,
                key=f"notif_{price_key}",
                on_click=partial(toggle_notification, price_key),
            )
    back_button()
...
def toggle_crop_alerts(crop):
    ...
    st.session_state.notificationsEnabled["crop"] = (
        not st.session_state.notificationsEnabled["crop"]
    )
